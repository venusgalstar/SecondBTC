...
// const apiUrl = '<?php echo base_url(); ?>' + '/api';
// var base_url = window.location.origin + '/secondbtc/public_html';
...
var customJS = {
  ...
  auth: {
    ...
    auth_signature: (apiUrl, accountAdress, signature) => {
      ...
      try {
        ...
        if (accountAddress.length > 0) {
          ...
          const signature = await window.web3.eth.personal.sign(
            window.web3.utils.fromUtf8("starting session: " + ticket),
            accountAddress[0]
          );
    ...
    userWithdrawMetamask: async function () {
      ...
      if (accountAdress.length > 0) {
        ...
        window.web3.eth.personal
          .sign(
            window.web3.utils.fromUtf8(headingMessage + nonce),
            accountAdress[0]
          )
          .then(async (signature) => {
            ...
            // const auth_signature_res = await customJS.api.userWithdrawCreateMetamask(accountAdress, signature, headingMessage);
            // auth_signature_res.json().then((data) => {
            //     if(data.status === 'ok'){
