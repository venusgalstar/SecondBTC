...
class Bot_model extends CI_Model {
	...
	{	
		...
		'exchange_bid' => (double)Number($fiyat,20),
	...
	{	
		...
		$this->mongo_db->where_gte('exchange_bid',(double)Number($bids,20));
	...
	{	
		...
		$this->mongo_db->limit(1);
		$this->mongo_db->where($arraybul);
		$this->mongo_db->where_lte('exchange_bid',(double)Number($bids,20));
		return $bul = $this->mongo_db->get('exchange_datas');
	}
	...
	{	
		...
		$this->mongo_db->where($delarray);
		$this->mongo_db->where_lte('exchange_bid',(double)Number($asks,20));
		$cancel = $this->mongo_db->delete_all('exchange_datas');
	}
	...
	{	
		...
		$this->mongo_db->limit(1);
		$this->mongo_db->where($arraybul);
		$this->mongo_db->where_gte('exchange_bid',(double)Number($asks,20));
		return $bul = $this->mongo_db->get('exchange_datas');
	}
	...
	{	
		...
		if($userId!="bot" && $durum==1){
			...
			$veri = array(
				...
				'exchange_user_id'=> (string)$userId,
				'exchange_user_email' => (string)$userEmail,
				'exchange_bid' => (double)Number($bids,20),
				'exchange_first_unit' => (double)Number($unit,20),
				'exchange_unit' => (double)Number($unit,20),
				'exchange_total' => (double)Number($unit*$bids,20),
	...
	{	
			if($bids>=0 && $unit>=0 && is_numeric($bids)){ 
			$veri = array(
				'exchange_id' => (string)uretken(28),
				'exchange_from_wallet_id' => (int)$fromID,
				'exchange_from_short' => (string)$from,
	...
	{	
		...
		$veri = array(
			...
			'trade_user_id'=> (string)"bot",
			'trade_user_email' => (string)"bot",
			'trade_bid' => (double)Number($bids,20),
			'trade_unit' => (double)Number($unit,20),
			'trade_total' => (double)Number($unit*$bids,20),
