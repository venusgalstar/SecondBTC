...
<?php 
    if($user_google_conf==1){
        ...
        $gosayfa = "twofa";}
    if($user_with_conf=="M"){$MWsecim = "selected"; }else{$MWsecim = "";}
    if($user_with_conf=="G"){$GWsecim = "selected"; }else{$GWsecim = "";}
    if($user_login_conf=="M"){$MLsecim = "selected"; }else{$MLsecim = "";}
    if($user_login_conf=="G"){$GLsecim = "selected"; }else{$GLsecim = "";}

?>
<div class="container">
    <div class="row mt-5">
        <div class="col-6"><h2 class="font-size-17"><?php echo lang("accountpage");?></h2></div>
    </div>
    <div class="row" id="myGroup">
        ...
        <div class="col-12 col-md-12 mt-5">
            <div class="collapse" id="options-tab" data-parent="#myGroup">
                <div class="card card-body">
                    <table class="table table-hover table-sm">
                        <thead>
                            <tr>
                                <th class="border-bottom-0"><h3 class="font-size-15"><?php echo lang("email");?> :</h3></th>
                                <td><h3 class="font-size-15"><?php echo $_SESSION['user_data'][0]['user_email']; ?></h3></td>
                            </tr>
                            <tr>
                                <th class="border-bottom-0"><h3 class="font-size-15"><?php echo lang("lastloginip");?> :</h3></th>
                                <td><h3 class="font-size-15"><?php echo $user_act{0}["act_ip"];?></h3></td>
                            </tr>
                        </thead>
                    </table>
            ...
            <div class="collapse text-center mt-2 mb-5" id="info-tab" data-parent="#myGroup">
                ...
                <?php if(!empty($userInfoDetail)){$Infodisabled = 'disabled="disabled"';}else{$Infodisabled = '';} ?>
                    <form>
                        <div class="form-group text-left">
                            <label for="user_first_name" ><?php echo lang("firsname");?> <i>(<?php echo lang("required");?>)</i></label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_first_name" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_first_name'];}else{echo '';} ?>">
                            <label for="user_middle_name" ><?php echo lang("middlename");?> </label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_middle_name" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_middle_name'];}else{echo '';} ?>">
                            <label for="user_last_name" ><?php echo lang("lastname");?> <i>(<?php echo lang("required");?>)</i></label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_last_name" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_last_name'];}else{echo '';} ?>">
                            ...
                            <option value="<?php if(!empty($userInfoDetail)){ echo $userInfoDetail{0}['user_country'];}else{echo '';} ;?>"><?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_country'];}else{echo '';} ?></option>
                                <?php $ttt = countries();   foreach ($ttt as $sehirler => $value) {?>
                                <option value="<?php echo $value; ?>"><?php echo $value; ?></option>
                                <?php } ?>
                            </select>
                            <label for="user_city" ><?php echo lang("city");?> </label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_city" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_city'];}else{echo '';} ?>">
                            <label for="user_district" ><?php echo lang("district");?> </label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_district" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_district'];}else{echo '';} ?>">
                            <label for="user_address" ><?php echo lang("address");?> </label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_address" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_address'];}else{echo '';} ?>">
                            <label for="user_id_number" ><?php echo lang("idnumber");?> <i>(<?php echo lang("required");?>)</i></label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_id_number" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_id_number'];}else{echo '';} ?>">
                            <label for="user_tel" ><?php echo lang("telnumber");?> <i>(<?php echo lang("required");?>)</i></label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_tel" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_tel'];}else{echo '';} ?>">
                            <label for="user_dogum" ><?php echo lang("birthdate");?> <i>(<?php echo lang("required");?>)</i></label>
                            <input type="text" class="form-control" <?=$Infodisabled?> id="user_dogum" value="<?php if(!empty($userInfoDetail)){echo $userInfoDetail{0}['user_dogum'];}else{echo '';};?>">
                        </div>
                ...
                <div class="col-md-6 offset-md-3 col-sm-12">
                    <form><!--
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="user_ip_value" value="<?php echo $user_act{0}["act_ip"];?>">
                        </div>
                        <div class="form-group">
