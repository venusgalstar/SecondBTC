...
class Piyasa extends CI_Controller {
	...
	public function piyasaBuy()
	{
		...
		if(!empty($fromShort) && !empty($toShort) && !empty($fromID) && !empty($toID) && !empty($apiWebSite)){
				...
				}elseif($apiWebSite=="pancakeswap" || $apiWebSite=="catetools-pancakeswap" || $apiWebSite=="catetools-uniswap"){
					if($botAction==2){

					}else{
						$enyuksekBids = $data*0.99;
						$unit = (rand(1,15).".".rand(100,999))*$volumeBot;
					}
				}else{
					if($botAction==2){
				...
				if(!empty($enyuksekBids)){
					...
					if(empty($aynı)){
						...
						if(empty($bul)){
							if($enyuksekBids>=0 && is_numeric($enyuksekBids)){
		...
		if(!empty($fromShort) && !empty($toShort) && !empty($fromID) && !empty($toID) && !empty($apiWebSite)){
				...
				if(!empty($endusukAsks)){
					...
					if(empty($aynı)){
						...
						if(empty($bul2)){
							if($endusukAsks>=0 && is_numeric($endusukAsks)){
	...
	public function apiSite($fromShort,$toShort,$apiWebSite)
	{
		...
		if($apiWebSite==="btcturk"){
			...
			$data = json_decode($veri,true);
			return $data["data"];
		}elseif($apiWebSite==="pancakeswap"){
			$wallet = walletHelper($toShort);
			$veri = vericek("https://api.pancakeswap.info/api/v2/tokens/".strtolower($wallet["wallet_cont"]));
			$data = json_decode($veri,true);
			if($fromShort=="BNB"){
				return $data["data"]["price_BNB"];
			}else{
				return $data["data"]["price"];
			}
		}elseif($apiWebSite==="catetools-pancakeswap"){
			$wallet = walletHelper($toShort);
			$veri = vericek("https://catetools.com/api/pancakeswap/v2/token/".strtolower($wallet["wallet_cont"]));
			$data = json_decode($veri,true);
			if($fromShort=="BNB"){
				return $data["data"]["price_BNB"];
			}else{
		...
		}elseif($apiWebSite==="catetools-uniswap"){
			$wallet = walletHelper($toShort);
			$veri = vericek("https://catetools.com/api/uniswap/v2/token/".strtolower($wallet["wallet_cont"]));
			$data = json_decode($veri,true);
			if($fromShort=="ETH"){
				return $data["data"]["price_ETH"];
			}else{
				return $data["data"]["price"];
			}
		}
	...
	public function piyasaTrade()
	{
		...
		if(empty($bul) || $userEmail==$bul{0}["exchange_user_email"]){
			...
			//$_SESSION["deneme"] = $sType." -  Fiyat : ".Number($fiyat,8)." Adet : ".$unit." - rand : ".$rand."<br>";
