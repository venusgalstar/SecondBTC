...
class Api extends CI_Controller
{
    ...
    public function tickers()
    {
        if ($_GET) {
            ...
            if (limit(1) == "ok") {
                header("Content-type: application/json; charset=utf-8");
                $dosya = '/var/www/Shared/web/secondbtc/apix.txt';
                include($dosya);
            } else {
                $json["errorDescription"] = limit(1);
                echo $bas = json_encode($json);
    ...
    public function tickersxyzabc()
    {
        ...
        if (!empty($data)) {
            ...
            echo $bas = json_encode($json);
            $dosyaadi = "/var/www/Shared/web/secondbtc/apix.txt"; //Dosyan?n bulundu?u dizin
    ...
    public function authRequest()
    {
        ...
        $user = $this->account_model->getUserByAddress(strtolower($address));
