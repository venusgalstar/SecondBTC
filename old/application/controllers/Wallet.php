    ...
    $data["withdrawHistory"] = array_map(
      function($withdraw) {
        if(strpos($withdraw["withraw_txid"], " ") == false && strlen($withdraw["withdraw_txid"]) > 0){
          $url = $withdraw['wallet_network'] == 1 || $withdraw['wallet_network'] == 4 ? "https://etherscan.io/tx/$withdraw[withdraw_txid]": "https://bscan.com/tx/$withdraw[withdraw_txid]"; // production
    ...
    $data["depositHistory"] = array_map(
      function($deposit) {
        if(strpos($deposit["dep_history_txid"], " ") == false && strlen($deposit["dep_history_txid"]) > 0){
          $url = $deposit['wallet_network'] == 1 || $deposit['wallet_network'] == 4 ? "https://etherscan.io/tx/$deposit[dep_history_txid]": "https://bscan.com/tx/$deposit[dep_history_txid]"; // production
