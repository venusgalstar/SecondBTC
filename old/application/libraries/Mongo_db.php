...
class Mongo_db
{
	...
	private function prepare()
	{
		...
		if (isset($this->config[$this->activate]) == TRUE) {
			if (empty($this->config[$this->activate]['hostname'])) {
				...
				$this->atlas = trim($this->config[$this->activate]['atlas']);
			}
			...
			if (
				isset($this->config[$this->activate]['no_auth']) == FALSE
			...
			if (
				empty($this->config[$this->activate]['read_preference']) ||
			...
			if (
				empty($this->config[$this->activate]['read_concern']) ||
	...
	private function connect()
	{
		...
		try {
			if ($this->atlas == TRUE) {
				$dns = "mongodb+srv://{$this->username}:{$this->password}@{$this->hostname}/{$this->database}";
				//$dns = "mongodb+srv://my_new_user741852:Uy6527499asfsadfsafav423@cluster0-1zdun.gcp.mongodb.net/mongo_database_my";
			} else {
