			...
			if(UserStatus($userID,$email,"user_ex_status")==1){
				...
				if($toWalletData{0}["wallet_ex_status"]==1){
					if($type=='buy'){
						...
						$total =Number(($unit*$price)*$commission,8);
						$yuvarla = ($total*1.999999)-$total;
						$amount =Number($yuvarla,8);
						$walletID = $fromID;

					}elseif($type=='sell'){
						$commission=$fromWalletData{0}["wallet_sell_com"];
						if(UserStatus($userID,$email,"user_free_trade")==1){$commission=1;} 
						$amount =Number($unit,8);
						$walletID = $toID;
					}else{
				...
				if($unit<$minUnit){
					return array('durum' => 'error', 'mesaj' => lang("minimumunit")." : ".Number($minUnit,8)." ".$toShort);
				}elseif($price<$minBid){
					return array('durum' => 'error', 'mesaj' => lang("minimumbid")." : ".Number($minBid,8)." ".$fromShort);
				}elseif((Number(($unit*$price)*$commission,8))<$minTotal){
					return array('durum' => 'error', 'mesaj' => lang("minimumtotal")." : ".Number($minTotal,8)." ".$fromShort);
				}else{
...
//Bot 
	...
	{	
		...
		if($eksilt>0){ $kontrol = $update->getModifiedCount(); $status = (int)1; $completed = null;
			...
			if(count($insert)){
				...
				.lang("total").' : '.Number($unit*$price,8).' '.$fromShort.'<br>'
				.lang("time").' : '.date("H:i:s").'<br>';
				return array('durum' => 'info', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email);
			}else{
				$this->logger->write( "exchangeError", "trade", "(".microtime().") open order(createOrderBook) işleminde  kullanıcının bakiyesi düşüldü ama emir açılmadı: ,exchange_user_id : ".$userID.", Miktar : ".Number($unit,8).", Fiyat : ".Number($price,8).", fromid : ".$fromID.", toid : ".$toID.", type : ".$type);
				return array('durum' => 'error', 'mesaj' => lang("hata").' 202');}
		}else{
			$this->logger->write( "exchangeError", "trade", "(".microtime().") open order(createOrderBook) işleminde  kullanıcının bakiyesi düşülmedi: ,exchange_user_id : ".$userID."  ,Miktar : ".Number($eksilt,8)."  ,type : ".$type."From".$fromShort." To".$toShort);
	...
	public function buyExchange($unit,$price,$fromID,$toID,$toShort,$fromShort,$type,$commission,$userID,$email,$sellOrder)
	{
		...
		if((double)$unit>=(double)$sellUnit){
			...
			$odenenTotal = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$commission,8);
			$saticiyaOde = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$sellCommission,8);
			$alicidanDus = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$commission,8);
			...
			if($saticiEmir->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde satıcının emri güncellenmedi : ,exchange_id : ".$sellID."  ,exchange_user_id : ".$sellUserId." ,Miktar : ".Number($odenenTotal,8)." From ".$fromShort." To ".$toShort);
			}
				...
				if($saticiFromWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde satıcı sattığının karşılığını almadı : ,exchange_id : ".$sellID."  ,exchange_user_id : ".$sellUserId."  ,Miktar : ".Number($sellTotal,8)."From".$fromShort." To".$toShort);
				};
				//alıcıya to coin öde
				$aliciToWallet = $this->UserWalletUpdateArttir($userID,$email,$toID,$alinanUnit);
				if($aliciToWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcı almak istediğini alamadı : ,exchange_id : ".$sellID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($alinanUnit,8)."From".$fromShort." To".$toShort);
				};
				...
				if($aliciFromWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcı aldığının karşılığını ödemedi : ,exchange_id : ".$sellID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($alinanUnit,8)."From".$fromShort." To".$toShort);
				}
					if($kalanUnit >0){
						...
						if(!empty($sellOrder)){
							...
							.lang("total").' : '.Number($alinanUnit*$sellPrice,8).' '.$fromShort.'<br>'
							.lang("time").' : '.date("H:i:s").'<br>';
							...
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu alıcı ".$sellUserId." id nolu satıcıdan ".Number($sellPrice,8)." fiyatından ".$unit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin almak istedi ama satıcının miktarı(".$sellUnit.") yetmedi ve emir tekrar etti.");
							...
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu alıcı ".$sellUserId." id nolu satıcıdan ".Number($sellPrice,8)." fiyatından ".$alinanUnit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin satın aldı. Satıcının miktarı yetmedi ve eşleşen bir fiyat olmadığı için ".$kalanUnit." adet alış emri açıldı.");
							return array('durum' => 'success', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email,'toid' => $sellUserId);
						}
					}else{
							$aciklama = 
							...
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu alıcı ".$sellUserId." id nolu satıcıdan ".Number($sellPrice,8)." fiyatından ".$alinanUnit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin satın aldı. Alıcı almak istediği miktarı aldı. Boş emir açıldı.");
							return array('durum' => 'success', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email,'toid' => $sellUserId);
					}
		}else{
			...
			$alinanUnit  = $unit;
			$odenenTotal = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$commission,8);
			$SkalanUnit = Number(((double)$sellUnit-(double)$unit),8);//unit bitti
			$SkalanTotal = Number(((double)$SkalanUnit*(double)$sellPrice),8);
			$saticiyaOde = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$sellCommission,8);
			$alicidanDus = Number(((double)$alinanUnit*(double)$sellPrice)*(double)$commission,8);
			...
			$aliciToWallet = $this->UserWalletUpdateArttir($userID,$email,$toID,$alinanUnit);
			if($aliciToWallet->getModifiedCount()!=1){
				$this->logger->write( "exchangeError", "trade", "(".microtime().") ".$aliciToWallet->getModifiedCount()." trade işleminde alıcı almak istediğini alamadı : ,exchange_id : ".$sellID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($alinanUnit,8)."From".$fromShort." To".$toShort);
			};
			//alıcının from coini azalt
			$aliciFromWallet = $this->UserWalletUpdateAzalt($userID,$email,$fromID,$alicidanDus);
			if($aliciFromWallet->getModifiedCount()!=1){
			...
			$this->tradeDataInsert($fromID,$toID,$toShort,$fromShort,$alinanUnit,$sellPrice,$odenenTotal,$userID,$email,$type,$sellUserEmail,'taker',$commission);
			$this->createOrderBook(0,$price,$fromID,$toID,$type,$commission,$userID,$email,$unit,$toShort,$fromShort);
			$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu alıcı ".$sellUserId." id nolu satıcıdan ".Number($sellPrice,8)." fiyatından ".$alinanUnit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin satın aldı. Emir kapandı.");
			return array('durum' => 'success', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email,'toid' => $sellUserId);
	...
	public function sellExchange($unit,$price,$fromID,$toID,$toShort,$fromShort,$type,$commission,$userID,$email,$buyOrder)
	{
		...
		if((double)$unit>=(double)$buyUnit){
			...
			$odenenTotal = Number(((double)$satilanUnit*(double)$buyPrice)*(double)$commission,8);
			$saticiyaOde = Number(((double)$satilanUnit*(double)$buyPrice)*(double)$commission,8);
			...
			$saticiEmir = $this->mongo_db->update('exchange_datas');
			if($saticiEmir->getModifiedCount()!=1){
				$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcının emri güncellenmedi : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$buyUserId." ,Miktar : ".Number($odenenTotal,8)." From ".$fromShort." To ".$toShort);
			}
				//alıcıya to coin öde
				...
				if($saticiFromWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcı almak istediğini alamadı : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$buyUserId."  ,Miktar : ".Number($buyTotal,8)."From".$fromShort." To".$toShort);
				};
				//satıcı from coin öde
				$aliciToWallet = $this->UserWalletUpdateArttir($userID,$email,$fromID,$saticiyaOde);
				if($aliciToWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde satıcı sattığının karşılığını alamadı : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($satilanUnit,8)."From".$fromShort." To".$toShort);
				};
				//satıcı to coini azalt
				$aliciFromWallet = $this->UserWalletUpdateAzalt($userID,$email,$toID,$satilanUnit);
				if($aliciFromWallet->getModifiedCount()!=1){
					$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde satıcı sattığı miktarı ödemedi : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($satilanUnit,8)."From".$fromShort." To".$toShort);
				}
					if($kalanUnit >0){
						...
						if(!empty($buyOrder)){
							...
							.lang("total").' : '.Number($satilanUnit*$buyPrice,8).' '.$fromShort.'<br>'
							.lang("time").' : '.date("H:i:s").'<br>';
							...
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu satıcı ".$buyUserId." id nolu alıcıya ".Number($buyPrice,8)." fiyatından ".$unit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coini satmak istedi ama alıcının miktarı(".$buyUnit.") yetmedi ve emir tekrar etti.");
							...
							$this->tradeDataInsert($fromID,$toID,$toShort,$fromShort,$satilanUnit,$buyPrice,$odenenTotal,$userID,$email,$type,$buyUserEmail,'taker',$commission);
							$this->createOrderBook($kalanUnit,$price,$fromID,$toID,$type,$commission,$userID,$email,$unit,$toShort,$fromShort);
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu satıcı ".$buyUserId." id nolu alıcıya ".Number($buyPrice,8)." fiyatından ".$satilanUnit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin sattı. Alıcının miktarı yetmedi ve eşleşen bir fiyat olmadığı için ".$kalanUnit." adet satış emri açıldı.");
							return array('durum' => 'success', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email,'toid' => $buyUserId);
						}
					}else{
							$aciklama = 
							...
							$this->tradeDataInsert($fromID,$toID,$toShort,$fromShort,$satilanUnit,$buyPrice,$odenenTotal,$userID,$email,$type,$buyUserEmail,'taker',$commission);
							$this->createOrderBook(0,$price,$fromID,$toID,$type,$commission,$userID,$email,$unit,$toShort,$fromShort);
							$this->logger->write( "exchange", "trade", "(".microtime().") trade işleminde  : ".$userID." id nolu satıcı ".$buyUserId." id nolu alıcıya ".Number($buyPrice,8)." fiyatından ".$satilanUnit." adet ".$fromID." id nolu coin karşılığında ".$toID." id nolu coin sattı. Emir kapandı.");
							return array('durum' => 'success', 'mesaj' => $aciklama,'islem' => 'orderbook','id' => $userID, 'hesap' => $email,'toid' => $buyUserId);
					}
		}else{
			$kalanUnit = 0;//unit bitti 
			$satilanUnit  = $unit;
			$odenenTotal = Number(((double)$satilanUnit*(double)$buyPrice)*(double)$buyCommission,8);
			$saticiyaOde = Number(((double)$satilanUnit*(double)$buyPrice)*(double)$commission,8);
			$BkalanUnit = Number(((double)$buyUnit-(double)$unit),8);//unit bitti
			$BkalanTotal = Number(((double)$BkalanUnit*(double)$buyPrice)*(double)$commission,8);
			//return array('durum' => 'success', 'mesaj' => 'işlem tamamlandi : '.$alinanUnit);
			...
			if($saticiFromWallet->getModifiedCount()!=1){
				$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde satıcı sattığının karşılığını almadı : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$buyUserId."  ,Miktar : ".Number($buyTotal,8)."From".$fromShort." To".$toShort);
			};
			//alıcıya to coin öde
			$aliciToWallet = $this->UserWalletUpdateArttir($userID,$email,$fromID,$saticiyaOde);
			if($aliciToWallet->getModifiedCount()!=1){
				$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcı almak istediğini alamadı : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($satilanUnit,8)."From".$fromShort." To".$toShort);
			};
			//alıcının from coini azalt
			$aliciFromWallet = $this->UserWalletUpdateAzalt($userID,$email,$toID,$satilanUnit);
			if($aliciFromWallet->getModifiedCount()!=1){
				$this->logger->write( "exchangeError", "trade", "(".microtime().") trade işleminde alıcı aldığının karşılığını ödemedi : ,exchange_id : ".$buyID."  ,exchange_user_id : ".$userID."  ,Miktar : ".Number($satilanUnit,8)."From".$fromShort." To".$toShort);
			}
			$aciklama = 
			lang($type).' '.lang("ordersucnow").'<br>'
			.lang("amount").' : '.$satilanUnit.' '.$toShort.'<br>'
