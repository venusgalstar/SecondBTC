...
class Wallet_model extends CI_Model
{
	...
	public function depositHistory($userID, $email)
  {
	 ...
	 * @return mixed
	 */
	public function depositCreate($walletShort, $userID, $email, $amount, $walletSystem, $bankaName, $IBAN, $islemCode)
	{
		$veri = array(
			'dep_history_id' => (string)uretken(28),
			'dep_history_address' => (string)strtoupper($IBAN),
			...
			'dep_history_tag' => (string)$bankaName,
		...
		if (count($insert)) {
			return 'ok';
			$this->load->model('email_model');
			$this->email_model->my_email_send("noreply@secondbtc.com", "User with " . $email . " mail address requested " . $walletShort . " FIAT deposit. Amount : " . $amount . " Please check.");
		}
	}
