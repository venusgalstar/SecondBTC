                    ...
                    if (!empty($adminemail) && !empty($adminpass)) {
                        ...
                        if (!empty($sor)) {
                            if ($sor[0]["admin_2fa_status"] == 0) {
                                ...
                                $kontrol = getUserGoogleCodeKontrol(yeniSifreCoz($sor[0]["admin_google_key"]), $admingoogle);
                                if ($kontrol == "ok") {
                                    $this->session->set_userdata('user_data_admin', $sor);
                                    $this->adminActivity($this->input->post("email"), "Login", "");
                                    $this->load->model('email_model');
                            ...
                            $this->session->set_flashdata('hata', 'The email or password is incorrect. Or such an account is not registered! ');
